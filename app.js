...
document.getElementById('switch-room').addEventListener('click', async ()=>{
  if(!confirm("Changer de salle ? Tu devras retaper un code d'accès (et éventuellement te reconnecter avec ton prénom et mot de passe si c'est une autre salle).")) return;

  // On coupe les écouteurs de la salle actuelle avant de tout réinitialiser.
  if(participantsUnsub) participantsUnsub();
  if(messagesUnsub) messagesUnsub();
  if(configUnsub) configUnsub();
  if(challengeUnsub) challengeUnsub();
  participantsUnsub = messagesUnsub = configUnsub = challengeUnsub = null;

  if(typeof authReady !== 'undefined' && authReady){
    try{ await auth.signOut(); }catch(e){ console.warn('Déconnexion impossible :', e); }
  }

  currentRoomId = null;
  currentRoomName = null;
  currentRoomAdminPassword = null;
  sharedParticipants = {};
  cloudMessagesCache = [];
